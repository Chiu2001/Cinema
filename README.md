# Project Name: Cinema

[![Java](https://img.shields.io/badge/language-Java-brightgreen)](https://www.oracle.com/java/)

## Project Overview
This project implements a movie ticket booking website, including a movie homepage, login, sign-up, seat selection and ticket purchasing, a shopping cart, third-party login, third-party payment (ECPay and Stripe), and an admin system, with the goal of improving the user's ticket-booking experience.


## System Architecture
1. Web Frontend
   - User login, Google login
   - Admin backend (publish/unpublish movies)
   - Movie seat selection
   - Third-party payment (ECPay, Stripe)

2. Backend
   - Seat reservation for ticket purchases
   - Spring Security for application security
   - RESTful API design
   - CRUD database operations

## Installation and Usage

### Requirements

- A computer running Windows, Mac, or Linux

### Installation Steps

Clone the project:

   ```bash
   git clone https://github.com/Chiu2001/Cinema
   cd Cinema
   ```

## Contact

If you have any questions or suggestions, feel free to reach out:
- Name: Chiu Cheng-Yi, Tsai Meng-En
- Email: your.email@example.com
- GitHub Issues: [Submit here](https://github.com/Chiu2001/Cinema/issues)

## Reference
- https://chatgpt.com/share/66eb98a2-c7cc-8004-bdda-5909133b5c49
